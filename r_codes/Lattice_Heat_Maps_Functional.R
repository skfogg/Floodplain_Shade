...
scaleslist <- list(y = list(at = c(38, 36, 34, 32), labels = c(1, 3, 5, 7)))
textlocation <- c(700,36.5)
maintitle <- "Hyporheic Zone Temperature"

whatraster1 <- soil_1.0m_sunny_raster
whatraster2 <- soil_1.0m_shady_raster

filepng <- "C:\\Users\\Katie Fogg\\Desktop\\HGSwork\\pretty_plots\\soil_1.0m_heatmap_hzonly\\heatmap_sunny_soil_1.0m_"
                            
## Does not work unless copy-pasty
## 1 ##
i = 1
png(paste0(filepng, i,".png"),
    res = 72*8,
    width = 800*8,
    height = 600*8)
stacktest <- stack(whatraster1[[i]],
                   whatraster2[[i]])
...
p1 <- levelplot(stacktest,
  ...
  layer(sp.text(loc = textlocation, txt = jframe$dayofyear[i]))
plot(p1)
