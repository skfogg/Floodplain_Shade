...
tempAcrossFP2.3 <- function(whichmodel, whichdaily, addmodel, adddaily, k, d2hz, bc, xrange = 2:49, colorpal = "RdYlGn", a = 0.5, ...){
  png(paste0(getwd(),"/plots/tempAcrossFP_version3_k", k, "_", d2hz, "_", bc,".png"),
      height = 400,
      width = 800)
  par(cex.lab = 1.8,
      cex.main = 2,
  ...
  mapply(function(t,c) lines(adddaily[xrange,2,30,1,"X"],
                             rowMeans(adddaily[xrange,2,aquifer_z_idx,t,"temp"]),
                             col = c,
                             lwd = 2),
         daytimesvector,
  ...
  mapply(function(t, c) lines(colMeans(addmodel[aquifer_z_idx,,t]) ~ get(paste0("model_", d2hz, "_xvals")), 
                              col = c,
                              ...),
         t_idx,
...
mapply(tempAcrossFP2.3,
       whichmodel = listofRiveronly,
       whichdaily = listofRiveronlydaily, 
       addmodel = listofShady,
       adddaily = listofShadydaily,
       k = kNames2,
       d2hz = modelNames2,
       bc = rep("riveronly", times = 8),
       MoreArgs = list(lwd = 3, a = 0.35))
